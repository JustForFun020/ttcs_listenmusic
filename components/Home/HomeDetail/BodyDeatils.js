        ...
        {_.map(music, (item) => {
          return (
            <View>
              {_.map(item, (song) => {
                const today = moment();
                const releaseDate = moment.unix(song.releaseDate);
                const duration = moment.duration(today.diff(releaseDate));
                const dayAgo =
                  Math.floor(duration.asDays()) === 0 ? 'Hôm nay' : `${Math.floor(duration.asDays())} ngày trước`;
                return (
                  <TouchableOpacity
                    key={song._id}
                  ...
                  >
                    <Image source={{ uri: song.thumbnail }} style={{ width: 80, height: 70, borderRadius: 10 }} />
                    <View style={{ paddingLeft: 10 }}>
                      <Text numberOfLines={1} style={{ width: 300 }}>
                        {song.title}
                      </Text>
                      <Text style={{ width: 250, opacity: 0.5 }} numberOfLines={1}>
                        {song.artistsNames}
                      </Text>
                      <Text style={{ opacity: 0.5 }}>Ngày phát hành: {dayAgo}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
  ...
  return (
    <SafeAreaView style={{ marginLeft: 10, marginRight: 10, marginBottom: 60 }}>
      <View style={styles.OptionMusiccontainer}>
        ...
        <TouchableOpacity style={styles.optionMusic} onPress={() => navigation.navigate('ChartContainer')}>
          <View style={[styles.iconWrapper, { backgroundColor: '#EEC23F' }]}>
            <AntDesign name='staro' size={24} color='white' />
          </View>
          <Text>Top 100</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.optionMusic} onPress={() => navigation.navigate('RadioContainer')}>
      ...
      <View style={styles.textNewSong}>
        <View style={{ marginBottom: 12 }}>
          <Text style={{ fontWeight: 'bold', fontSize: 18 }}>
            Mới Phát Hành <FontAwesome name='chevron-right' size={16} color='black' />
          </Text>
        </View>
          ...
          {_.map(newSongTypes, (item, k) => {
            return (
              <TouchableOpacity
                ...
                style={activeType === k ? styles.activeType : styles.newSongType}
                key={item}
              >
                <Text style={activeType === k ? styles.textActive : styles.textType}>{item}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
